...
-- O BEFORE apenas solta o link; a linha do ledger é apagada num AFTER UPDATE,
-- quando a rental já está gravada (senão: "tuple already modified").
create or replace function public.fn_surf_rental_ledger()
returns trigger language plpgsql security definer set search_path to 'public' as $$
declare
  new_ledger_id uuid; loc_slug text; ledger_category text; v_date date;
begin
  if tg_op = 'UPDATE' and old.is_returned = true and new.is_returned = false then
    new.ledger_entry_id := null;   -- a linha é apagada pelo AFTER trigger
    return new;
  end if;
  if new.ledger_entry_id is not null then return new; end if;
  if new.kind = 'rental' and not (new.is_returned = true and (tg_op = 'INSERT' or old.is_returned = false)) then
...
create or replace function public.fn_surf_rental_reopen_cleanup()
returns trigger language plpgsql security definer set search_path to 'public' as $$
begin
  if old.is_returned = true and new.is_returned = false and old.ledger_entry_id is not null then
    perform set_config('app.hq_mirror','1',true);
    delete from public.ledger_entries where id = old.ledger_entry_id;
  end if;
  return null;
end $$;
drop trigger if exists tr_surf_rental_reopen_cleanup on public.surf_school_rentals;
